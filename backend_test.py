...
class FieldHockeyConnectAPITest(unittest.TestCase):
    ...
    def test_sequence(cls):
        ...
        if response.status_code == 200:
            ...
            if "filename" in avatar_data and avatar_data["message"] == "Avatar uploaded successfully":
                print("✅ Avatar upload test passed")
            else:
                print("❌ Avatar upload test failed: Incorrect response")
                return
        else:
        ...
        if response.status_code == 200:
            ...
            if "filename" in cv_data and cv_data["message"] == "CV uploaded successfully":
                print("✅ CV upload test passed")
            else:
                print("❌ CV upload test failed: Incorrect response")
                return
        else:
        ...
        if response.status_code == 200:
            ...
            if "filename" in photo_data and photo_data["message"] == "Photo uploaded successfully":
                print("✅ Photo upload test passed")
                
                # Get updated player to get the photo ID
                response = requests.get(f"{BASE_URL}/players/{cls.player_id}")
                if response.status_code == 200:
                    player = response.json()
                    if player["photos"] and len(player["photos"]) > 0:
                        cls.photo_id = player["photos"][0]["id"]
                        print(f"✅ Retrieved photo ID: {cls.photo_id}")
                    else:
                        print("❌ Failed to retrieve photo ID")
                        return
                else:
                    print(f"❌ Failed to get updated player: {response.status_code} - {response.text}")
                    return
            else:
                print("❌ Photo upload test failed: Incorrect response")
                return
        else:
        ...
        if response.status_code == 200:
            ...
            if "filename" in video_data and video_data["message"] == "Video uploaded successfully":
                ...
                if response.status_code == 200:
                    player = response.json()
                    if player["videos"] and len(player["videos"]) > 0:
                        cls.video_id = player["videos"][0]["id"]
                        print(f"✅ Retrieved video ID: {cls.video_id}")
                    else:
                ...
                print("❌ Video upload test failed: Incorrect response")
                return
        else:
        ...
        if cls.photo_id:
            ...
            if response.status_code == 200:
                ...
                if delete_data["message"] == "Photo deleted successfully":
                    print("✅ Photo deletion test passed")
                else:
                    print("❌ Photo deletion test failed: Incorrect response")
                    return
            else:
        ...
        if cls.video_id:
            ...
            if response.status_code == 200:
                delete_data = response.json()
                if delete_data["message"] == "Video deleted successfully":
                    print("✅ Video deletion test passed")
                else:
                    print("❌ Video deletion test failed: Incorrect response")
                    return
            else:
